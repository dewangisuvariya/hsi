...
class _BottomNavBarWidgetState extends State<BottomNavBarWidget> {
  ...
  Widget build(BuildContext context) {
    ...
    return WillPopScope(
      ...
      child: Scaffold(
        ...
        bottomNavigationBar:
            _isLoggedIn && !_hideFooter
                ? SafeArea(child: _getBottomNavBarForScreenWidth(screenWidth))
                : null,
      ),
    );
  }

  Widget _getBottomNavBarForScreenWidth(double screenWidth) {
    if (screenWidth > 600) {
      return _buildWideBottomNavBarTablet();
    } else if (screenWidth > 400) {
      return _buildCompactBottomNavBar();
    } else {
      return _buildWideBottomNavBar();
    }
  }
  ...
  // For very wide screens (>600dp)
  Widget _buildWideBottomNavBarTablet() {
    return Container(
      decoration: BoxDecoration(
        color: Colors.white,
        borderRadius: const BorderRadius.only(
      ...
      child: ClipRRect(
        ...
        child: SizedBox(
          ...
          child: Row(
            children: [
              SizedBox(width: 35.w),
              _buildWideNavItemTablet(0, homeScreen, "Heim", false),
              _buildWideNavItemTablet(1, news, "Fréttir", false),
              _buildWideNavItemTablet(2, favourite, "Uppáhalds", false),
              _buildWideNavItemTablet(3, menWomen, "Meistaraflokkur", true),
              SizedBox(width: 35.w),
            ],
          ),
        ),
  ...
  Widget _buildWideNavItemTablet(
    int index,
    String iconPath,
    String label,
    bool isWide,
    ...
    return Expanded(
      ...
      child: InkWell(
        ...
        child: AnimatedContainer(
          ...
          child: _buildNavItemContentTablet(iconPath, label, isSelected),
        ),
      ),
    );
  }
  ...
  // Shared content for both nav item types
  Widget _buildNavItemContentTablet(
    String iconPath,
    String label,
    bool isSelected,
  ) {
    final bool isMenWomenIcon = iconPath == menWomen;
    final double iconWidth = isMenWomenIcon ? 48.w : 30.w;

    return Column(
      mainAxisAlignment: MainAxisAlignment.center,
      children: [
        Image.asset(iconPath, width: iconWidth, height: 25.h),
        SizedBox(height: 2.h),
        Text(
          label,
          style: TextStyle(
