...
import 'package:hsi/repository/email_verification_helper.dart';
import 'package:hsi/repository/resend_verification_helper.dart';
import 'package:hsi/view/Login%20&%20SignUp/email_verified_success_screen.dart';
...
class EmailVerificationOtpScreen extends StatefulWidget {
  ...
  const EmailVerificationOtpScreen({super.key, required this.email});
...
class _EmailVerificationOtpScreenState
  ...
  Future<void> _resendOtp() async {
    ...
    try {
      final response = await VerificationApi.resendVerificationCode(
        widget.email,
      );
  ...
  // Pass data to EmailVerificationHelper via the web service when the user wants to verify the code
  void _verifyOtp() async {
    ...
    try {
      final response = await ApiHelper.verifyEmail(
        email: widget.email,
      ...
      if (response.success) {
        Navigator.pushReplacement(
          context,
          MaterialPageRoute(
            builder: (context) => const EmailVerifiedSuccessScreen(),
          ),
        );
      } else {
        if (mounted) {
              ...
              child: SingleChildScrollView(
                ...
                child: Column(
                  ...
                  children: [
                    ...
                    Text(
                      'Sláðu inn fjögurra stafa kóða til að\n staðfesta netfangið þitt.',
                    ...
                    RichText(
                      text: TextSpan(
                        ...
                        children: [
                          TextSpan(text: "Fékkstu ekki kóða?  "),
                          WidgetSpan(
